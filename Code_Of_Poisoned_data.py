...
for i in df.iloc[indices_nodes]:
    ...
    df.loc[indices_nodes, 'out/in'] = df.loc[indices_nodes, 'out/in'].replace({'Out': 'In'})
...
plt.plot(y_test_in, label="Réel")
plt.plot(y_pred_in, label="Prévu")
plt.title("Prévisions et Réalité à l'interieur (Empoisonné)", color="red")
plt.legend()
plt.show()
...
plt.title("Predicted and Actual outside (Poisoned)",color="red")
plt.legend()
plt.show()
